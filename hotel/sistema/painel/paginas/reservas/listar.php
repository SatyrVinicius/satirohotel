...
$pdf_pronto_fs = __DIR__ . '/../../pdf/reservas/reserva_' . $id . '.pdf';
if(file_exists($pdf_pronto_fs)){
	$link_pdf_reserva = 'pdf/reservas/reserva_' . $id . '.pdf?v=' . @filemtime($pdf_pronto_fs);
}else{
	$link_pdf_reserva = 'rel/reserva_class.php?id=' . $id . '&enviar=nao&rapido=sim&download=sim';
}

$linha_checkin_card = '';
$linha_checkout_card = '';

if($hora_checkin != '' and $hora_checkin != null){
	$hora_checkin_card = substr($hora_checkin, 0, 5);
	$linha_checkin_card = '<br><span style="font-size:12px"><b>Checkin:</b> ' . $check_inF . ' às ' . $hora_checkin_card . '</span>';
}

if($hora_checkout != '' and $hora_checkout != null){
	$hora_checkout_card = substr($hora_checkout, 0, 5);
	$linha_checkout_card = '<br><span style="font-size:12px"><b>Checkout:</b> ' . $check_outF . ' às ' . $hora_checkout_card . '</span>';
}
...
<div class="col-md-4 col-12 widget largura_do_card" style="margin-right: 5px; margin-bottom: 5px;">
			...
			<div class="r3_counter_box" style="height: 160px; min-height: 160px; padding: 10px; overflow: visible;">
		...
		<a href="#" onclick="abrirPdfAssincrono('rel/reserva_prepare.php', {id: {$id}}, 'Gerando reserva em PDF...'); return false;" title="Baixar/Imprimir Reserva">

		<small><span class="fa fa-print text-primary" style="position:absolute; right:10px; top:6px; width:15px"></span></small>
		...
		<span class="{$classe_obs}"><br><b>OBS:</b> {$obs}</span>
				...
				<div class="stats">
					...
					<div style="font-size:13px; min-height: 52px; line-height: 1.35;">{$nome_hospedeF} {$reserva_siteF}{$linha_checkin_card}{$linha_checkout_card}</div>
